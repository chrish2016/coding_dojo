...
SELECT percentage, language, countries.name
FROM languages
JOIN countries ON countries.id = languages.country_id
WHERE percentage > 89
ORDER BY percentage DESC;
...
SELECT government_form, name, capital, life_expectancy
FROM countries
...
SELECT cities.name, cities.population, cities.district, countries.name
FROM cities
JOIN countries ON countries.id = cities.country_id
